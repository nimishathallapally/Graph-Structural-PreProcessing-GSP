#!/usr/bin/env python3
"""
run_ml1m.py  –  MovieLens-1M paper-results pipeline
====================================================

Mirrors run_yelp_1m.py but uses the MovieLens-1M dataset.
Uses proper Leave-One-Out split (hold out exactly 1 positive per user)
and evaluates ranking metrics at k = 10, 20, 50.

Typical usage
-------------
    # All four models, 100 epochs
    python scripts/run_ml1m.py --epochs 100

    # LightGCN only, quick test
    python scripts/run_ml1m.py --models lightgcn --epochs 30

    # Custom output dir
    python scripts/run_ml1m.py --output_dir outputs/ml1m_run1 --epochs 100
"""
...
def _parse_args() -> argparse.Namespace:
    p = argparse.ArgumentParser(
        description="MovieLens-1M paper-results pipeline",
        formatter_class=argparse.ArgumentDefaultsHelpFormatter,
    )
    p.add_argument("--output_dir", default="outputs/ml1m",
    ...
    p.add_argument("--implicit_threshold", type=float, default=4.0,
                   help="Minimum rating to treat as a positive interaction")
    p.add_argument("--eval_k", type=int, default=10,
                   help="Primary eval k (used in progress prints; all of 10/20/50 always computed)")
    ...
    p.add_argument("--er_solver", default="dwlv",
                   ...
                   help="ER solver. 'dwlv' = O(nnz) closed-form, fastest. "
                        "'jl' = JL-sketch MINRES. 'arpack' = exact but slow.")
    p.add_argument("--er_sketches", type=int, default=32)
    p.add_argument("--max_cluster_size", type=int, default=50,
                   help="Maximum users per super-node (50 enables 20-30%% compression)")
    p.add_argument("--clustering_method", default="hem",
                   ...
                   help="Clustering algorithm: 'hem' (default) = Heavy-Edge Matching, "
                        "avoids giant-component collapse on dense graphs.")
    p.add_argument("--min_shared", type=int, default=2)
    p.add_argument("--max_item_degree", type=int, default=0,
                   help="Exclude items with more than this many users from UU similarity (0=no cap)")
    p.add_argument("--max_neighbors_per_user", type=int, default=0,
                   help="Per-user top-K cap in UU graph (0=no cap, auto=100 for large datasets)")
...
def _split_leave_one_out(
    df, threshold: float, seed: int = 42
) -> Tuple[Any, Any]:
...
def _eval_multi_k(
    ...
    """Evaluate ranking metrics at multiple cutoffs."""
...
def _print_paper_table(rows: List[Dict], ks: Tuple[int, ...] = _EVAL_KS) -> None:
    ...
    print("\n" + "=" * len(header))
    print("PAPER RESULTS TABLE  —  MovieLens-1M")
    print("=" * len(header))
    print(header)
    print("-" * len(header))
    for r in rows:
...
def main() -> None:
    ...
    print(f"[ML1M] Device  : {device}")
    print(f"[ML1M] Models  : {models_to_run}")
    print(f"[ML1M] Epochs  : {args.epochs}")
    if args.target_fraction is not None:
        print(f"[ML1M] Target  : top {args.target_fraction*100:.1f}% most active users")
    else:
        print(f"[ML1M] Target  : full dataset (no user filtering)")
    print(f"[ML1M] Output  : {out_dir}")
    ...
    print(
        f"[ML1M] {num_users:,} users | {num_items:,} items | "
    ...
    if args.target_fraction is not None:
        ...
        print(f"[ML1M] Subset  : {_subset_method_label}")
        print(
            f"[ML1M] After filter: {num_users:,} users | {num_items:,} items | "
    ...
    print(
        f"[ML1M] LOO split | train={len(train_df):,} | test={len(test_df):,} | "
    ...
    print(f"[ML1M] \u250c\u2500 Dataset \u2500\u2500\u2500\u2500\u2500\u2500\u2500\u2500\u2500\u2500\u2500\u2500\u2500\u2500\u2500\u2500\u2500\u2500\u2500\u2500\u2500\u2500\u2500\u2500\u2500\u2500\u2500\u2500\u2500\u2500\u2500\u2500\u2500\u2500\u2500\u2500\u2500\u2500\u2500\u2500\u2500\u2500\u2500\u2500\u2500\u2500\u2500\u2500\u2500\u2500\u2500\u2500\u2500\u2500\u2500\u2500\u2500\u2500\u2500\u2500")
    print(f"[ML1M] \u2502  Users / Items / Interactions     : {num_users:>14,} / {num_items:,} / {n_interactions:,}")
    print(f"[ML1M] \u2502  Positive train interactions      : {_n_pos_train:>14,}")
    print(f"[ML1M] \u2502  Train / Test split               : {len(train_df):>14,} / {len(test_df):,}")
    print(f"[ML1M] \u251c\u2500 Training \u2500\u2500\u2500\u2500\u2500\u2500\u2500\u2500\u2500\u2500\u2500\u2500\u2500\u2500\u2500\u2500\u2500\u2500\u2500\u2500\u2500\u2500\u2500\u2500\u2500\u2500\u2500\u2500\u2500\u2500\u2500\u2500\u2500\u2500\u2500\u2500\u2500\u2500\u2500\u2500\u2500\u2500\u2500\u2500\u2500\u2500\u2500\u2500\u2500\u2500\u2500\u2500\u2500\u2500\u2500\u2500")
    print(f"[ML1M] \u2502  Batch size                        : {args.batch_size:>14,}")
    print(f"[ML1M] \u2502  Epochs                            : {args.epochs:>14,}")
    print(f"[ML1M] \u2502  Steps / epoch  ceil({_n_pos_train:,}/{args.batch_size:,})  : {_steps_per_epoch:>14,}")
    print(f"[ML1M] \u2502  Total optimisation steps          : {_total_opt_steps:>14,}  ({args.epochs} epochs \u00d7 {_steps_per_epoch} steps)")
    print(f"[ML1M] \u2502  LR scheduler                      :  ReduceLROnPlateau(patience=2, factor=0.5, min_lr=1e-5)")
    print(f"[ML1M] \u251c\u2500 Subset Selection \u2500\u2500\u2500\u2500\u2500\u2500\u2500\u2500\u2500\u2500\u2500\u2500\u2500\u2500\u2500\u2500\u2500\u2500\u2500\u2500\u2500\u2500\u2500\u2500\u2500\u2500\u2500\u2500\u2500\u2500\u2500\u2500\u2500\u2500\u2500\u2500\u2500\u2500\u2500\u2500\u2500\u2500\u2500\u2500\u2500\u2500\u2500\u2500")
    print(f"[ML1M] \u2502  Method                            :  {_subset_method_label}")
    print(f"[ML1M] \u2502  Final user count                  : {num_users:>14,}")
    print(f"[ML1M] \u2502  Final interaction count           : {n_interactions:>14,}")
    print(f"[ML1M] \u2514\u2500\u2500\u2500\u2500\u2500\u2500\u2500\u2500\u2500\u2500\u2500\u2500\u2500\u2500\u2500\u2500\u2500\u2500\u2500\u2500\u2500\u2500\u2500\u2500\u2500\u2500\u2500\u2500\u2500\u2500\u2500\u2500\u2500\u2500\u2500\u2500\u2500\u2500\u2500\u2500\u2500\u2500\u2500\u2500\u2500\u2500\u2500\u2500\u2500\u2500\u2500\u2500\u2500\u2500\u2500\u2500\u2500\u2500\u2500\u2500\u2500\u2500\u2500\u2500\u2500\u2500\u2500\u2500\u2500\u2500\u2500\u2500\u2500\u2500\u2500\u2500\u2500\u2500")
    ...
    print(
        f"[ML1M] GSP done in {gsp_elapsed:.1f}s | "
    ...
    print(
        f"[ML1M] Baseline graph : {nodes_orig:,} nodes | {edge_index_base.shape[1]:,} edge-slots"
    )
    print(
        f"[ML1M] GSP graph      : {nodes_gsp:,} nodes | {edge_index_gsp.shape[1]:,} edge-slots"
    ...
    for model_name in models_to_run:
        ...
        # ── Baseline ──────────────────────────────────────────────────────────
        print(f"[ML1M] {model_name} | Baseline  ({nodes_orig:,} nodes, {edges_orig:,} edges)")
        try:
            base_model = get_model(model_name, nodes_orig, model_cfg)
        except Exception as exc:
            print(f"[ML1M] WARNING: cannot build '{model_name}': {exc}  Skipping.")
            continue
        ...
        # ── GSP ───────────────────────────────────────────────────────────────
        print(f"\n[ML1M] {model_name} | GSP  ({nodes_gsp:,} nodes, {edges_gsp:,} edges)")
        try:
            gsp_model = get_model(model_name, nodes_gsp, model_cfg)
        except Exception as exc:
            print(f"[ML1M] WARNING: cannot build GSP '{model_name}': {exc}  Skipping GSP.")
            continue
        ...
        try:
            ...
            print(f"[ML1M] WARNING: analytics failed for {model_name}: {_analytics_exc}")
    # ─────────────────────────────────────────────────────────────────────────
    # Final output
    # ─────────────────────────────────────────────────────────────────────────
    ...
    print(f"\n[ML1M] Total wall-clock time : {total_wall/60:.1f} min")
    print(f"[ML1M] Results written to    : {out_dir}/")
    print(f"[ML1M]   results_table.csv")
    print(f"[ML1M]   speedup_results.csv")
    print(f"[ML1M]   full_results.json")
